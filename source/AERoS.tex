...
The behaviours of a swarm are not explicitly engineered. Instead, they are an emergent consequence of the interaction of individual agents with each other within the swarm and their environment.
This emergent functionality poses a challenge to \emph{assurance}, such as safety assurance.
The main contribution of this paper is a process for the safety assurance of emergent behaviour for use in autonomous robotic swarms called \emph{AERoS}, following the guidance on the Assurance of Machine Learning for use in Autonomous Systems (AMLAS). 
We explore our proposed guidance using an illustrative case study centred on a robot swarm operating a public cloakroom.
...
% Dhaminda: This is addressed now
...
However, although these industry standards focus on ensuring the assurance of robots at the individual level, they do not cover safety or any other extra-functional property at the \emph{swarm} level that may arise through emergent behaviour (EB). %, which provides motivation for our study.
...
%\vspace{-2ex}
AS with evolving functionality are considered to follow much more iterative life-cycle compared to the conventional V\&V model. 
...
\begin{figure}[!t]
	...
	\begin{minipage}{.5\textwidth}
		...
		\vspace{-2ex}
		\caption{Stage 1: The AERoS EB assurance scoping process.}
		\label{amlas-a-stage1}
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{figures/stage1-failureevents-v3.png}
		\vspace{-2ex}
...
\begin{figure}[!t]
	...
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=0.99\textwidth]{figures/amlas-a-stage2-v3.png}
		\vspace{-2ex}
		\caption{Stage 2: The AERoS EB safety requirements assurance.}% process.}
	\label{amlas-a-stage2}
\end{minipage}%
\begin{minipage}{.55\textwidth}
	\centering
	\includegraphics[width=0.9\textwidth]{figures/3Dsim.png}
	\vspace{-2ex}
	\caption{3D simulation created to validate several EB safety requirements.}
	\label{3Dsim}
\end{minipage}
...
%These requirements may be fulfilled by constraining the maximum velocity of individual robots. To future-proof, , or by making this future-proof a robot has one or more sensory devices, such as a camera or infra-red sensors enabling it to detect obstacles. 
...
\emph{[EE] Operational Scenarios:} These operational scenarios should reflect the environment descriptions specified in [B], offering real-world situations to examine the behaviour of the integrated system. The testing of the integrated system in these true-to-operation environments should be conducted in a safe manner. 
...
First, the scope of this work has been limited to looking at inherent swarm qualities and the adaptation that arises from these. 
However, one can expand on this, and consider machine learning for individual robots, for example by applying AMLAS. 
Second, in this work, we explored the AERoS process by using only a single case study, which can be broadened by considering additional swarm use cases (e.g. monitoring fires in a natural environment, and a social swarm), and a worked example of the entire AERoS process.
