...
The behaviours of a swarm are not explicitly engineered. Instead, they are an emergent consequence of the interaction of individual agents within the swarm with each other and their environment.
This emergent functionality poses a challenge to \emph{assurance}, such as safety assurance.
The main contribution of this paper is a process for safety assurance of emergent behaviour for use in autonomous robotic swarms called \emph{AERoS}, following the Assurance of Machine Learning for use in Autonomous Systems (AMLAS) guidance. 
We explore our proposed guidance using an illustrative case study centred on a robot swarm operating a public cloakroom.
...
The main contribution of this paper is a process or guidance for the safety assurance of EB for use in autonomous robotic swarms (AERoS), adapted from the Assurance of Machine Learning for use in Autonomous Systems (AMLAS) process~\cite{Hawkins2021} proposed for machine learning systems. 
...
There are several standards and guidance related to machine learning in aeronautics, automotive, railway and industrial domains \cite{Kaakai2022}, e.g. AMLAS process~\cite{Hawkins2021}, European Union Aviation Safety Agency (EASA) concept paper, DEpendable and Explainable Learning (DEEL) white paper, Aerospace Vehicle System Institute (AVSI) report, Laboratoire National de Métrologie et d'Essais (LNE) certification, and UL 4600 standard. None of the aforementioned standards and guidance target swarm systems.

In this work we used AMLAS \cite{Hawkins2021} as the foundation for developing an assurance process for autonomous robotic swarms. \emph{AMLAS} provides guidance on how to systematically integrate safety assurance into the development of the machine learning components based on offline supervised learning. 
...
\begin{table}[!t]
	...
	\begin{tabular}{p{5mm} p{116mm} }
		...
		*Assumption: Agents are considered stationary once they have not moved for $>$ \textbf{10 seconds}. \\
		...
		\hline
	\end{tabular}
	\caption{\label{tab:reqs}Example safety requirements for the cloakroom scenario.}
	\vspace{-4ex}
\end{table}
%\normalsize
...
\begin{table}[!h]
	...
	\begin{tabular}{p{6mm} p{116mm}}
		...
		\hline
	\end{tabular}
	\caption{\label{tab:human-s}Example human safety requirements for the cloakroom scenario.}
	\vspace{-4ex}
\end{table}   
%\normalsize
...
%\vspace{-2.5ex}
The authors would like to thank the fruitful insights and comments provided by John Downer, Jonathan Ives and Alvin Wilby. 
